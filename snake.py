...
class Player():
    def __init__(self,display_width,display_height):
        self.dis_w = display_width
        self.dis_h = display_height
        self.posx = self.dis_w/2
        self.posy = self.dis_h/2
        self.body = []
        ...
        if len(self.body)>=1:
            for i in range(1,len(self.body)):
                if self.body[0] == self.body[i]:
                    return True
        if (self.posx>=self.dis_w-self.size
            or self.posx<=0
            or self.posy>=self.dis_h-self.size
            or self.posy <= 0):
            return True
        else:
            return False
    ...
    def draw(self, display, color):
        ...
        for bodypart in self.body:
            pygame.draw.rect(display, color,(bodypart[0], bodypart[1], self.size, self.size))
...
class Food():
    ...
    def placeFood(self):
        if not self.isEdible:
            self.posx = random.randint(1, self.dis_w/10-1)*10
            self.posy = random.randint(1, self.dis_h/10-1)*10
            self.isEdible = True;
...
snake = Player(dis_w, dis_h)
food = Food(dis_w, dis_h)
